...
{
  DECLARE_BITMAP(bitmap, MAX_RAS_PRIO + 1); /* include 1 bit for delimiter */
...
static inline void prepare_lock_switch(struct rq *rq,
...
extern int update_runtime(struct notifier_block *nfb, unsigned long action,
                          void *hcpu);
...
{
  return container_of(task_subsys_state(tsk, cpuacct_subsys_id), struct cpuacct,
                      css);
}
