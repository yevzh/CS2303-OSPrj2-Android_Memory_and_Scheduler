...
static inline void list_add_leaf_cfs_rq(struct cfs_rq *cfs_rq)
{
  if (!cfs_rq->on_list)
  {
    ...
    {
      list_add_rcu(&cfs_rq->leaf_cfs_rq_list, &rq_of(cfs_rq)->leaf_cfs_rq_list);
    }
    else
    {
...
static inline int is_same_group(struct sched_entity *se,
...
static inline int depth_se(struct sched_entity *se)
{
  ...
  for_each_sched_entity(se) depth++;
 ...
 * delta /= w
 */
static inline unsigned long calc_delta_fair(unsigned long delta,
                                            struct sched_entity *se)
{
...
static inline long calc_tg_weight(struct task_group *tg,
 ...
 */
static int select_task_rq_fair(struct task_struct *p, int sd_flag,
                               int wake_flags)
{
                                                 ...
                                                 struct sg_lb_stats *sgs)
{
   ...
   */
  if (local_group &&
      (sds->this_nr_running >= sgs->group_capacity || !sds->this_nr_running))
    sds->power_savings_balance = 0;
   ...
   */
  if (!sds->power_savings_balance ||
      sgs->sum_nr_running >= sgs->group_capacity || !sgs->sum_nr_running)
    return;
       ...
       group_first_cpu(group) > group_first_cpu(sds->group_min)))
  {
    ...
    sds->min_load_per_task = sgs->sum_weighted_load / sgs->sum_nr_running;
  }
...
/**
 * check_power_save_busiest_group - see if there is potential for some
 ...
 */
static inline int fix_small_capacity(struct sched_domain *sd,
                                     struct sched_group *group)
{
  /*
                                      ...
                                      struct sg_lb_stats *sgs)
{
  ...
  sgs->group_capacity = DIV_ROUND_CLOSEST(group->sgp->power, SCHED_POWER_SCALE);
...
static bool update_sd_pick_busiest(struct sched_domain *sd,
                                   struct sd_lb_stats *sds,
                                   struct sched_group *sg,
                                   struct sg_lb_stats *sgs, int this_cpu)
{
                                      ...
                                      struct sd_lb_stats *sds)
{
  ...
  do
  {
    ...
    update_sg_lb_stats(sd, sg, this_cpu, idle, load_idx, local_group, cpus,
                       balance, &sgs);
 ...
 */
static int check_asym_packing(struct sched_domain *sd, struct sd_lb_stats *sds,
 ...
 * @imbalance: Variable to store the imbalance.
 */
static inline void fix_small_imbalance(struct sd_lb_stats *sds, int this_cpu,
                                       unsigned long *imbalance)
{
  ...
  if (sds->this_nr_running)
  {
    ...
    if (sds->busiest_load_per_task > sds->this_load_per_task)
      imbn = 1;
  }
  else
    sds->this_load_per_task = cpu_avg_load_per_task(this_cpu);

  scaled_busy_load_per_task = sds->busiest_load_per_task * SCHED_POWER_SCALE;
  ...
  pwr_now +=
      sds->busiest->sgp->power * min(sds->busiest_load_per_task, sds->max_load);
  pwr_now +=
      sds->this->sgp->power * min(sds->this_load_per_task, sds->this_load);
                                       ...
                                       unsigned long *imbalance)
{
  ...
  if (sds->group_imb)
  {
    sds->busiest_load_per_task = min(sds->busiest_load_per_task, sds->avg_load);
  }
...
redo:
  group = find_busiest_group(sd, this_cpu, &imbalance, idle, cpus, balance);
  ...
  if (busiest->nr_running > 1)
  {
    ...
    env.src_rq = busiest;
    env.loop_max =
        min_t(unsigned long, sysctl_sched_nr_migrate, busiest->nr_running);
...
static inline struct sched_domain *lowest_flag_domain(int cpu, int flag)
{
  ...
  for_each_domain(cpu, sd) if (sd->flags & flag) break;
...
#define for_each_flag_domain(cpu, sd, flag)                            \
  for (sd = lowest_flag_domain(cpu, flag); (sd && (sd->flags & flag)); \
       sd = sd->parent)
...
static int find_new_ilb(int cpu)
{
  ...
  for_each_flag_domain(cpu, sd, SD_POWERSAVINGS_BALANCE)
  {
    ...
    do
    {
      if (ilbg->group_weight != atomic_read(&ilbg->sgp->nr_busy_cpus))
      {
        ilb = cpumask_first_and(nohz.idle_cpus_mask, sched_group_cpus(ilbg));
        goto unlock;
      }
...
void set_cpu_sd_state_idle(void)
{
  ...
  rcu_read_lock();
  for_each_domain(cpu, sd) atomic_dec(&sd->groups->sgp->nr_busy_cpus);
  rcu_read_unlock();
}
...
static void rq_offline_fair(struct rq *rq) { update_sysctl(); }
...
int sched_group_set_shares(struct task_group *tg, unsigned long shares)
{
  ...
  for_each_possible_cpu(i)
  {
    ...
    raw_spin_lock_irqsave(&rq->lock, flags);
    for_each_sched_entity(se) update_cfs_shares(group_cfs_rq(se));
    raw_spin_unlock_irqrestore(&rq->lock, flags);
  }
