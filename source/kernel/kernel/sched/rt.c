...
int alloc_rt_sched_group(struct task_group *tg, struct task_group *parent)
{
  ...
  init_rt_bandwidth(&tg->rt_bandwidth, ktime_to_ns(def_rt_bandwidth.rt_period),
                    0);
...
static void inc_rt_migration(struct sched_rt_entity *rt_se,
                             struct rt_rq *rt_rq)
{
...
static void dec_rt_migration(struct sched_rt_entity *rt_se,
                             struct rt_rq *rt_rq)
{
  if (!rt_entity_is_task(rt_se))
    return;
...
{
  list_add_rcu(&rt_rq->leaf_rt_rq_list, &rq_of_rt_rq(rt_rq)->leaf_rt_rq_list);
}
...
static inline void inc_rt_prio_smp(struct rt_rq *rt_rq, int prio,
                                   int prev_prio)
{
}
static inline void dec_rt_prio_smp(struct rt_rq *rt_rq, int prio,
                                   int prev_prio) {}
...
static void enqueue_rt_entity(struct sched_rt_entity *rt_se, bool head)
{
  ...
  for_each_sched_rt_entity(rt_se) __enqueue_rt_entity(rt_se, head);
}
...
static int select_task_rq_rt(struct task_struct *p, int sd_flag, int flags)
{
  ...
  if (curr && unlikely(rt_task(curr)) &&
      (curr->rt.nr_cpus_allowed < 2 || curr->prio <= p->prio) &&
  ...
  for (tries = 0; tries < RT_MAX_TRIES; tries++)
  {
    ...
    if (double_lock_balance(rq, lowest_rq))
    {
      ...
      if (unlikely(task_rq(task) != rq ||
                   !cpumask_test_cpu(lowest_rq->cpu, tsk_cpus_allowed(task)) ||
...
static void post_schedule_rt(struct rq *rq) { push_rt_tasks(rq); }
...
static void task_woken_rt(struct rq *rq, struct task_struct *p)
{
  if (!task_running(rq, p) && !test_tsk_need_resched(rq->curr) &&
      has_pushable_tasks(rq) && p->rt.nr_cpus_allowed > 1 &&
      rt_task(rq->curr) &&
