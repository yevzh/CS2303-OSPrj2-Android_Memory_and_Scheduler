...
void start_bandwidth_timer(struct hrtimer *period_timer, ktime_t period)
{
  ...
  for (;;)
  {
    ...
    __hrtimer_start_range_ns(period_timer, soft, delta, HRTIMER_MODE_ABS_PINNED,
                             0);
  }
}
...
static ssize_t sched_feat_write(struct file *filp, const char __user *ubuf,
 ...
 */
static struct rq *this_rq_lock(void) __acquires(rq->lock)
{
  struct rq *rq;
...
static int hotplug_hrtick(struct notifier_block *nfb, unsigned long action,
                          void *hcpu)
{
...
static __init void init_hrtick(void) { hotcpu_notifier(hotplug_hrtick, 0); }
#else
/*
 * Called to set the hrtick timer state.
 *
 ...
 */
static inline int select_task_rq(struct task_struct *p, int sd_flags,
                                 int wake_flags)
{
...
static void try_to_wake_up_local(struct task_struct *p)
{
  ...
  if (WARN_ON_ONCE(rq != this_rq()) || WARN_ON_ONCE(p == current))
    return;
...
unsigned long nr_uninterruptible(void)
{
  ...
  for_each_possible_cpu(i) sum += cpu_rq(i)->nr_uninterruptible;
...
unsigned long nr_iowait(void)
{
  ...
  for_each_possible_cpu(i) sum += atomic_read(&cpu_rq(i)->nr_iowait);
...
static inline int calc_load_read_idx(void) { return calc_load_idx & 1; }
 ...
 *             i=0          1 - x
 */
static unsigned long calc_load_n(unsigned long load, unsigned long exp,
...
static const unsigned char degrade_zero_ticks[CPU_LOAD_IDX_MAX] = {0, 8, 32, 64,
                                                                   128};
static const unsigned char degrade_factor[CPU_LOAD_IDX_MAX][DEGRADE_SHIFT + 1] =
    {{0, 0, 0, 0, 0, 0, 0, 0},
     {64, 32, 8, 0, 0, 0, 0, 0},
     {96, 72, 40, 12, 1, 0, 0},
     {112, 98, 75, 43, 15, 1, 0},
 ...
 */
static inline void __account_system_time(struct task_struct *p,
                                         cputime_t cputime,
...
static cputime_t scale_utime(cputime_t utime, cputime_t rtime,
                             cputime_t total)
{
...
static inline bool owner_running(struct mutex *lock,
...
static long __sched sleep_on_common(wait_queue_head_t *q, int state,
                                    long timeout)
{
  unsigned long flags;
  wait_queue_t wait;
 ...
 * @p: the task in question.
 */
int task_nice(const struct task_struct *p) { return TASK_NICE(p); }
...
recheck:
  ...
  else
  {
    ...
    if (policy != SCHED_FIFO && policy != SCHED_RR && policy != SCHED_NORMAL &&
        policy != SCHED_BATCH && policy != SCHED_IDLE && policy != SCHED_RAS)
      return -EINVAL;
  }
 ...
 */
SYSCALL_DEFINE2(sched_rr_get_interval, pid_t, pid, struct timespec __user *,
                interval)
{
  struct task_struct *p;
...
void sched_show_task(struct task_struct *p)
{
  ...
  printk(KERN_CONT "%5lu %5d %6d 0x%08lx\n", free, task_pid_nr(p),
         task_pid_nr(rcu_dereference(p->real_parent)),
...
static void set_table_entry(struct ctl_table *entry, const char *procname,
                            void *data, int maxlen, umode_t mode,
...
static struct ctl_table *sd_alloc_ctl_domain_table(struct sched_domain *sd)
{
  ...
  set_table_entry(&table[0], "min_interval", &sd->min_interval, sizeof(long),
                  0644, proc_doulongvec_minmax);
  set_table_entry(&table[1], "max_interval", &sd->max_interval, sizeof(long),
                  0644, proc_doulongvec_minmax);
  set_table_entry(&table[2], "busy_idx", &sd->busy_idx, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[3], "idle_idx", &sd->idle_idx, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[4], "newidle_idx", &sd->newidle_idx, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[5], "wake_idx", &sd->wake_idx, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[6], "forkexec_idx", &sd->forkexec_idx, sizeof(int),
                  0644, proc_dointvec_minmax);
  set_table_entry(&table[7], "busy_factor", &sd->busy_factor, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[8], "imbalance_pct", &sd->imbalance_pct, sizeof(int),
                  0644, proc_dointvec_minmax);
  set_table_entry(&table[9], "cache_nice_tries", &sd->cache_nice_tries,
                  sizeof(int), 0644, proc_dointvec_minmax);
  set_table_entry(&table[10], "flags", &sd->flags, sizeof(int), 0644,
                  proc_dointvec_minmax);
  set_table_entry(&table[11], "name", sd->name, CORENAME_MAX_SIZE, 0444,
                  proc_dostring);
...
static ctl_table *sd_alloc_ctl_cpu_table(int cpu)
{
  ...
  for_each_domain(cpu, sd) domain_num++;
...
static int sd_degenerate(struct sched_domain *sd)
{
  ...
  if (sd->flags &
      (SD_LOAD_BALANCE | SD_BALANCE_NEWIDLE | SD_BALANCE_FORK |
       SD_BALANCE_EXEC | SD_SHARE_CPUPOWER | SD_SHARE_PKG_RESOURCES))
  {
                                ...
                                struct sched_domain *parent)
{
  ...
  {
    pflags &= ~(SD_LOAD_BALANCE | SD_BALANCE_NEWIDLE | SD_BALANCE_FORK |
                SD_BALANCE_EXEC | SD_SHARE_CPUPOWER | SD_SHARE_PKG_RESOURCES);
    if (nr_node_ids == 1)
      pflags &= ~SD_SERIALIZE;
  }
...
static int build_overlap_sched_groups(struct sched_domain *sd, int cpu)
{
  ...
  for_each_cpu(i, span)
  {
    ...
    sg = kzalloc_node(sizeof(struct sched_group) + cpumask_size(), GFP_KERNEL,
                      cpu_to_node(cpu));
...
int __weak arch_sd_sibling_asym_packing(void) { return 0 * SD_ASYM_PACKING; }
...
#define SD_INIT_FUNC(type)                                    \
  static noinline struct sched_domain *sd_init_##type(        \
      struct sched_domain_topology_level *tl, int cpu)        \
  {                                                           \
  ...
  for (tl = sched_domain_topology; tl->init; tl++)
  {
    ...
    for_each_cpu(j, cpu_map)
    {
      ...
      *per_cpu_ptr(sdd->sg, j) = sg;
      ...
      sgp = kzalloc_node(sizeof(struct sched_group_power), GFP_KERNEL,
                         cpu_to_node(j));
      if (!sgp)
        return -ENOMEM;
 ...
 */
int __attribute__((weak)) arch_update_cpu_topology(void) { return 0; }
                        ...
                        struct sched_domain_attr *new, int idx_new)
{
  ...
  tmp = SD_ATTR_INIT;
  return !memcmp(cur ? (cur + idx_cur) : &tmp, new ? (new + idx_new) : &tmp,
...
static DEVICE_ATTR(sched_mc_power_savings, 0644, sched_mc_power_savings_show,
...
static DEVICE_ATTR(sched_smt_power_savings, 0644, sched_smt_power_savings_show,
...
void sched_destroy_group(struct task_group *tg)
{
  ...
  for_each_possible_cpu(i) unregister_fair_sched_group(tg, i);
...
static int cpu_cgroup_allow_attach(struct cgroup *cgrp,
                              ...
                              struct cgroup_taskset *tset)
{
  ...
  cgroup_taskset_for_each(task, cgrp, tset) sched_move_task(task);
}

static void cpu_cgroup_exit(struct cgroup *cgrp, struct cgroup *old_cgrp,
                            struct task_struct *task)
{
  /*
...
static u64 cpuusage_read(struct cgroup *cgrp, struct cftype *cft)
{
  ...
  for_each_present_cpu(i) totalcpuusage += cpuacct_cpuusage_read(ca, i);
                          ...
                          u64 reset)
{
  ...
  for_each_present_cpu(i) cpuacct_cpuusage_write(ca, i, 0);
