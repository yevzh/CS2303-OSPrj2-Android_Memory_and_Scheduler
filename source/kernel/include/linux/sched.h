...
#else
static inline void proc_sched_show_task(struct task_struct *p,
                                        struct seq_file *m)
{
}
static inline void proc_sched_set_task(struct task_struct *p) {}
static inline void print_cfs_rq(struct seq_file *m, int cpu,
...
extern char
    ___assert_task_state[1 - 2 * !!(sizeof(TASK_STATE_TO_CHAR_STR) - 1 !=
...
#define task_contributes_to_load(task) \
  ((task->state & TASK_UNINTERRUPTIBLE) != 0 && (task->flags & PF_FROZEN) == 0)
 ...
 * current task), SP is the stack pointer of the first frame that should be
 * shown in the back trace (or NULL if the entire call-chain of the task should
 * be shown).
 */
...
#define INIT_CPUTIME \
  (struct task_cputime) { .utime = 0, .stime = 0, .sum_exec_runtime = 0, }
...
#ifdef CONFIG_EPOLL
  atomic_long_t
      epoll_watches; /* The number of file descriptors currently watched */
#endif
...
#define SD_SHARE_PKG_RESOURCES \
  0x0200                       /* Domain members share cpu pkg resources */
#define SD_SERIALIZE 0x0400    /* Only a single load balancing instance */
#define SD_ASYM_PACKING 0x0800 /* Place busy groups earlier in the domain */
#define SD_PREFER_SIBLING                                              \
  0x1000                  /* Prefer to place tasks in a sibling domain \
                           */
...
#define SD_ATTR_INIT \
  (struct sched_domain_attr) { .relax_domain_level = -1, }
...
struct task_struct
{
  ...
  unsigned int wcounts;    /*trace the write fault*/
  unsigned int trace_flag; /*whether tracing*/
...
#define MAX_USER_RAS_PRIO 100
#define MAX_RT_PRIO MAX_USER_RT_PRIO
#define MAX_RAS_PRIO MAX_USER_RAS_PRIO
...
extern void thread_group_times(struct task_struct *p, cputime_t *ut,
                               cputime_t *st);
...
#define PF_FREEZER_SKIP \
  0x40000000 /* Freezer should not count it as freezable */
...
#define conditional_stopped_child_used_math(condition, child) \
  ...
  {                                                           \
    (child)->flags &= ~PF_USED_MATH,                          \
        (child)->flags |= (condition) ? PF_USED_MATH : 0;     \
  } while (0)
...
#define copy_to_stopped_child_used_math(child)           \
  ...
  {                                                      \
    (child)->flags &= ~PF_USED_MATH,                     \
        (child)->flags |= current->flags & PF_USED_MATH; \
  } while (0)
...
extern int do_execve(const char *, const char __user *const __user *,
...
static inline int same_thread_group(struct task_struct *p1,
