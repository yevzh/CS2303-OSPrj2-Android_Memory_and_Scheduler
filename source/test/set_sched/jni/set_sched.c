/*
This is the test program for problem3: set scheduler
Use a switch to choose a desired scheduling policy for a selected task
Employ the sched_setscheduler to change its scheduling policy
...
static void change_scheduler()
{
  ...
  // If the first time for RAS, we start trace
  // If not the first time, return its wcounts
  ...
  // set the task's priority, only effective for rt scheduling
  ...
  // syscall, to change the scheduler
  ret = sched_setscheduler(pid, policy, &param);
  if (ret < 0)
  {
    perror("Changing scheduler failed. Aborting...");
